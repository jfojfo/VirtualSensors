 ...
 */
#define  SENSORS_SERVICE_NAME "virtual_sensors"

#define LOG_TAG "VirtualSensors"
...
#define SENSOR_SOCK_PATH "/data/vsensors"
...
static void accept_clients(struct sensors_poll_device_t *dev)
{
    SensorPoll*  ctl = (void*)dev;
    int server_fd = ctl->fd;
    int epollfd = ctl->events_fd;
...
//    while (!quit) {
    {
        nready = epoll_wait(epollfd, events, MAXEVENTS, 1000);
        if (nready == -1) {
            D("epoll_wait failed: %s\n", strerror(errno));
//            break;
        }
        for (i = 0; i < nready; ++i) {
            if (events[i].events & EPOLLIN) {
                if (events[i].data.fd == server_fd) {
                    alen = sizeof(peeraddr);
                    fd = accept(server_fd, (struct sockaddr *) &peeraddr, &alen);
                    if (fd < 0) {
                        D("accept failed: %s\n", strerror(errno));
                    } else {
                        D("new client %d", fd);
                    ...
                    fd = events[i].data.fd;
                    int count;
                    char *edge = ctl->buffer + sizeof(ctl->buffer);
                    char *pos = ctl->end;
                    while (edge - pos > 0 && (count = read(fd, pos, edge - pos)) > 0) {
                        pos += count;
                    }
                    ctl->end = pos;
                    if (pos == edge) {
                        char *start = ctl->begin;
                        D("reach edge [%d, %d)", start - ctl->buffer, sizeof(ctl->buffer));
                        if (start > ctl->buffer) {
                            int len = pos - start;
                            D("memcpy %d bytes", len);
                            memcpy(ctl->buffer, start, len);
                            ctl->begin = ctl->buffer;
                            ctl->end = ctl->buffer + len;
                        }
                    }
                    
                    // The remote has closed the connection
                    if (count == 0) {
                        D("client %d read 0", fd);
                        ...
                        if (errno == EAGAIN) {
                            ...
                            D("client %d read err: %s", fd, strerror(errno));
                            epoll_ctl(epollfd, EPOLL_CTL_DEL, fd, &events[i]);
                            client_fd = -1;
                        }
                    }
                ...
                D("EPOLLERR or EPOLLHUP: %d, %s", events[i].events, strerror(errno));
...
static int start_server(struct sensors_poll_device_t *dev)
{
    int ret;
    SensorPoll*  ctl = (void*)dev;
    D("%s: %s", __FUNCTION__, SENSOR_SOCK_PATH);
    
    ctl->fd = socket_local_server(SENSOR_SOCK_PATH);
    D("server socket: %d", ctl->fd);
    if (ctl->fd < 0) {
        D("start server fail: %s", strerror(errno));
        return -1;
    }
    fcntl(ctl->fd, F_SETFD, FD_CLOEXEC);

//    pthread_create(&thread, NULL, accept_clients, &ctl->fd);
    
    int epollfd = epoll_create(MAXEVENTS);
    
    struct epoll_event event;
    event.data.fd = ctl->fd;
    event.events = EPOLLIN;
    epoll_ctl(epollfd, EPOLL_CTL_ADD, ctl->fd, &event);

    ctl->events_fd = epollfd;
    D("epoll fd: %d", epollfd);
...
//    D("%s: data=%p", __FUNCTION__, dev);
    ...
    char log[256];
    char *end = data->end;
    char *buff = data->begin;
    char *pos = buff;
    
#if 0
    {
        *end = '\0';
        D("[%d,%d) %s", buff - data->buffer, end - data->buffer, buff);
    }
#endif

    for (; buff < end; buff = pos + 1) {
        /* read the next event */
//        int      len = qemud_channel_recv(data->events_fd, buff, sizeof buff-1);
        ...
        if (sscanf(buff, "orientation:%g:%g:%g", params+0, params+1, params+2) == 3) {
            data->newSensors |= SENSORS_ORIENTATION;
        ...
        if (sscanf(buff, "magnetic:%g:%g:%g", params+0, params+1, params+2) == 3) {
            data->newSensors |= SENSORS_MAGNETIC_FIELD;
        ...
        if (sscanf(buff, "temperature:%g", params+0) == 1) {
            data->newSensors |= SENSORS_TEMPERATURE;
        ...
        if (sscanf(buff, "proximity:%g", params+0) == 1) {
            data->newSensors |= SENSORS_PROXIMITY;
        ...
        if (sscanf(buff, "sync:%lld", &event_time) == 1) {
            if (data->newSensors) {
                data->pendingSensors = data->newSensors;
                ...
                while (data->newSensors) {
                    uint32_t i = 31 - __builtin_clz(data->newSensors);
                    data->newSensors &= ~(1<<i);
...
//    D("%s: dev=%p data=%p count=%d ", __FUNCTION__, dev, data, count);
    ...
    if (datadev->fd < 0) {
        start_server((void *) datadev);
    }
    // no buf data left or buf data is incomplete
    if (datadev->begin == datadev->end || *(datadev->end - 1) != '\n') {
        accept_clients(dev);
    }
